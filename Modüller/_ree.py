#regular expression
# özel bir ifade tanımlıyoruz, kullanıcıdan aldığımız input o formata uyuyorsa kaydeder
# re module ile arama yapacağız
...
# re.findall()
result = re.findall("Python",str) #yazılanın virgül sonrasındakinde kaç tane bulunduğunu söyler
...
result = re.sub(" ","-",str)
result = re.sub("\\s","-",str)
#boşluk yerine diğer değeri koyuyor.

# re.search()
result = re.search("Python", str) #ilk 'Python'un indexlerini verir. (match metodu olarak)
